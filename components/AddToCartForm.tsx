...
export function AddToCartForm({ product }: AddToCartFormProps) {
  ...
  const handleAddToCart = () => {
    addItem({
      ...
      name: product.name,
      price: finalPrice,
      image: product.images[0] || "/logo/chucha-avatar.jpg",
          ...
          <div className="flex flex-wrap gap-2">
            {product.colors.map(color => (
              <button
                ...
                className={`px-4 py-2 border-2 rounded-xl font-bold text-xs transition-colors cursor-pointer ${selectedColor === color
          ...
          <div className="flex flex-wrap gap-2">
            {product.sizes.map(size => (
              <button
                ...
                className={`px-4 py-2 border rounded-xl font-bold text-xs transition-colors cursor-pointer ${selectedSize === size
                    ? "bg-accent-red text-white border-accent-red"
